...
//	console.log("showClassTable()スタート");
	...
	$('#dg').datagrid({
        ...
        onUncheck:unChecktest,//チェックが外されたとき
...
//	markers.length=0;
	...
	rdfmgr.executeSparql({//RDFmgrに検索式を渡して検索実行を要求
		...
		error:function(eType, eMsg, eInfo){
			alert(eType+"<br>"+eMsg+"<br>"+eInfo);
			alert("エラーが発生しました。初期化します。必要の場合更新願います");
			$('select#qsel3 option').remove();
			deleteMarker();
			clearMsglist();
			prepareShowTable();
...
function json_analyze(json){
	...
	console.log("json_analyzeスタート @@@@@ 222");
	console.log(datacheck);
	console.log("json_analyzeスタート @@@@@ 333");
	$("#edittable").empty();
	console.log("json_analyzeスタート @@@@@ 444");
	obj = new Object();
	var nomap = null;
