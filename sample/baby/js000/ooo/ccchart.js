  ...
  return {
    ...
    preProcessing: function (op, callback) {
      ...
      this.useFirstToColName =
        op.config.useFirstToColName || this.gcf.useFirstToColName;
      this.useFirstToColName =
      ...
      this.useFirstToRowName =
        op.config.useFirstToRowName || this.gcf.useFirstToRowName;
      ...
      */
      ...
      if (this.type === 'scatter') {
        ...
        if (this.useRow0thToHanrei === 'yes')
          ...
          this.hanreiNames =
            this.util.uniq(this.colNames.slice(1));
      ...
      this.wsThrowData = this.op.config.wsThrowData || this.gcf.wsThrowData || false;
      //遅延データを無視する場合に遅延と判定するミリ秒
      this.maxDelayMS = this.op.config.maxDelayMS || this.gcf.maxDelayMS || 100;
      ...
      //水平目盛り線AxisXの本数
      this.axisXLen = this.op.config.axisXLen || this.gcf.axisXLen || 10;
      ...
      //Y目盛を小数点有りにするか？ yScaleDecimal は _useDecimalに書き換えてください。ただし将来なくなります
      ...
      if(this.useShadow === 'yes'){
       ...
       this._ondrew =  function(){
         this.drawing = false;
         if (this.useCss === 'yes') this.adjustCss('drew', this.id, this.type);
    ...
    drawAxisX: function () {
      this.axisXWidth = this.op.config.axisXWidth || this.gcf.axisXWidth || 1;
      //水平目盛線を下辺から描いていく
      for (
    ...
    drawXscale: function (left, count){
      ...
      var yOffset =
        this.op.config.xScaleYOffset || this.gcf.xScaleYOffset || 18;
      var tOffset =
    ...
    drawUnit: function(unit){
      ...
      var left =
        this.chartLeft - (this.op.config.unitXOffset || this.gcf.unitXOffset || 10);
      var top =
    ...
    drawVals: function () {
      ...
      this.ctx.save();
      var xOffset =
        this.valXOffset = this.op.config.valXOffset || this.gcf.valXOffset || 10;
      var yOffset =
        this.valYOffset = this.op.config.valYOffset || this.gcf.valYOffset || 0;
      var font = this.valFont = this.op.config.valFont || this.gcf.valFont;
     // var sumCols = this.util.getSumCols(this);
      ...
      for (var k = 0; k < this.dataRowLen; k++) {
        ...
        for (var l = 0; l < this.data[k].length; l++) {
          ...
          var percent = this.util.mkPercentVal(this, this.data[k][l] , k, l);
          var val = this.data[k][l];
          val = (this.percentVal === 'yes')? percent : val;
          this.ctx.beginPath();
   ...
   //    console.log(that.id,x, y, row, col, data, colorSet, colorIndex)
        ...
        setTimeout(function(){
         ...
         // console.log(that.cssTooltips[id],height, borderTop, borderBottom, paddingTop, paddingBottom)
          ...
          that.hybridCss.insertRule(
              '#-ccchart-css-tooltip-'+id+':before {top:'+ ((borderBottom + paddingTop + paddingBottom)  + height) +'px}'
          )
        }, 0);
