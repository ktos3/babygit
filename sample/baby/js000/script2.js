...
//					console.log("あああ");
					...
					if(id==num-1){//                                  ここでcheckboxに表示するためのJSONデータ（文字列を生成）
						namejson += '{"classname":"<div id='+id+'>'+str+'</div>"}]';
...
//	console.log("showClassTable()スタート");
	...
	$('#dg').datagrid({
        ...
        onUncheck:unChecktest,//チェックが外されたとき 3がとりあえず動く
...
//	console.log(rowsnum);
	...
	for(var i=0;i<num;i++){//checkされてる分
		...
		for(var j=0;j<rowsnum;j++){//クラス数分
//			console.log("aaaaaaaaaaaaaaa"+check[i].classname);
//			console.log("bbbbbbbbbbbbb"+rows[j].classname);
...
//			eval(j);
			...
			pingindex[title]=j;
			title = $('#'+j).html();
			var str = title + "<img src=ping/dot"+j+".png>";
			$('#'+j).html(str);
			propetyclassname=title;
			$('#query33').val("SELECT * WHERE{?x rdf:type lodcu:" + title +".?x ?pro ?z.}order by ?x");//検索式を設定する
			rdfmgr.executeSparql({//RDFmgrに検索式を渡して検索実行を要求
...
function onCheckAll(rows){
	...
	deleteMarker();
	$('select#qsel3 option').remove();
//	alert("チェックオールの動作には対応しておりません。地図を初期化します。");
...
function showQueryResult2(index,data) {
	console.log("showQueryResult2()スタート @@@ script2");
	var index2 = index+1;
//	pingindex=index;
	delcheck=false;
	pingnum="ping/dot"+index+".png";//表示するピンの色指定
	console.log("showクエ2の"+pingnum);
	check 	=	$('#dg').datagrid("options").singleSelect;//trueならひとつ、falaseなら複数  if文でマーカーを保持するか消すかの二者択一に使用
	console.log(data);
	console.log( $('#'+index).html());
	var title = $('#'+index).html();
	if(check) markerReset();

//		console.log(title);
//		console.log(check);
	var str = title + "<img src=ping/dot"+index+".png>";
	$('#'+index).html(str);


	if(check){//ひとつ選択　前のを消す
		$('select#qsel3 option').remove();
		if(markers!=null){
...
function json_analyze(json){
	console.log("json_analyzeスタート @@@@@@@@@@@@@@@@@@   script2 ");
...
//	console.log(datacheck);
	...
	console.log("@@@@@  script2 @@@@  nomap ="+nomap);
...
//				console.log(check[classnamenum]);
//				console.log(pingindex);
//				console.log(check[classnamenum].classname);
//				console.log(delcheck);
				if(delcheck==true){
					cname=check[classnamenum].classname;
					pingcname=pingindex[cname];
					pingnum="ping/dot"+pingcname+".png";//表示するピンの色指定
//					console.log(pingnum);
				}
//				console.log(pingindex[cname]);
...
//	$('#qsel3').append($("<option></option>").val(obj[setting_name]).html(obj[setting_name]));
	...
	var table = maketable2(false,obj,num);
	var projson = new String('{"rows":[');
	var propertygrid = makejson(projson,obj,num);
...
//		gMarkerClusterer = new MarkerClusterer(mapObj);//----------------------------大量のデータを取り扱うときに便利だよ用
	...
	var image=new google.maps.MarkerImage(
			  ...
			  new google.maps.Size(48.0,64.0),  //画像のサイズ
			  new google.maps.Point(0,0),       //画像のposition
			  new google.maps.Point(0,0.0)     //画像のアンカーの位置
			);
		var shadow=new google.maps.MarkerImage(
				"ping/shadow.png",                     //影の画像のURL
				new google.maps.Size(81.0,64.0),  //影の画像のサイズ
...
function maketable2(nomap,obj,num){//propatygridテスト用
//	console.log("maketable()スタート");
...
function makejson(projson,obj,num){
	console.log(propetyclassname);
	for(var i in obj){
		if(i.search("名前")!="-1"){
			projson +='{"名前プロパティ":"'+obj[i]+'","group":'+propetyclassname+'},';
//			console.log(i);
//			console.log(obj[i]);
		}
	}
